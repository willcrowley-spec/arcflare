  ...
  return (
    <section className="mb-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wide text-navy-900">Investment range</h4>
          <p className="mt-1 text-xs text-slate-500">
            Expected cost reflects a pilot/MVP. Conservative case includes enterprise hardening.
          </p>
        </div>
        {modelVersion ? (
          <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-700 ring-1 ring-inset ring-slate-200">
            {modelVersion}
          </span>
        ) : null}
      </div>
        ...
        <table className="w-full border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-xs font-bold uppercase tracking-wide text-slate-500">
              ...
              <th className="py-2 text-right font-semibold">Investment</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-slate-100">
              <td className="py-2.5 pr-4 font-medium text-navy-900">Pilot / MVP</td>
              <td className="py-2.5 text-right font-semibold tabular-nums text-navy-900">{formatCurrency(range.pilot_mvp)}</td>
            </tr>
            <tr className="border-b border-slate-100">
              <td className="py-2.5 pr-4 font-medium text-navy-900">Expected</td>
              <td className="py-2.5 text-right font-semibold tabular-nums text-navy-900">{formatCurrency(range.expected)}</td>
            </tr>
            <tr>
              <td className="py-2.5 pr-4 font-medium text-navy-900">Enterprise hardening</td>
              <td className="py-2.5 text-right font-semibold tabular-nums text-navy-900">{formatCurrency(range.enterprise_hardened)}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-slate-200">
              <p className="font-semibold text-slate-500">Native touchpoints</p>
              <p className="mt-1 text-lg font-semibold tabular-nums text-navy-900">{Number.isFinite(nativeCount) ? nativeCount : 0}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-slate-200">
              <p className="font-semibold text-slate-500">External integrations</p>
          ...
          <dl className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-1.5 text-xs">
            {Object.entries(components).map(([key, value]) => (
              <Fragment key={key}>
                <dt className="text-slate-500">{INVESTMENT_COMPONENT_LABELS[key] ?? humanizeKey(key)}</dt>
